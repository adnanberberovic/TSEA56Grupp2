...
void TURN_Back(int mode)
{
	...
	else if (mode == 5) // vid mål och start
	{
				if( offset_ > 20)
		{
			MOTOR_RotateLeft(180 - angle_);
	...
	if ( (MAP_operatingMode_ == 0) && !MAP_rotating_ && !MAP_movingForward_)
	{
		...
		if (MAP_array[posY_][posX_].visited && (MAP_array[posY_][posX_].description == 5))
		{
			...
			else if(fatIf_ <= 0)
			{
				...
				goto Phase3; // Exit this phase			//Går till phase 3 men kommer alltid gå till phase 2
			}
...
void tape_Home()
{
	...
	if (MAP_array[15][15].description != 5) // inte korsning innan åka lite längre
	{
		MOTOR_Forward(50);
		JUNCTION_delay(6);
	}
	...
	LCD_SetPosition(16);
	LCD_SendString("X: ");
	LCD_display_uint8(MAP_currentPos[1]);
	_delay_ms(250);
	_delay_ms(250);
	_delay_ms(250);
	_delay_ms(250);
	...
	//MAP_moveForward(); //Works if we have a corridor in front of start...?
...
void tape_Junction()
{
	...
	if(goal_counter == 0) // Om man ska sätta att det är mål där fösta gången, move_forward är INTE gjord
	{
		Goal_found = 1;
		ReflexSensor = 0;	
